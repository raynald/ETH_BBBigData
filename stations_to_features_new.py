        ...
        if code in codeSet:
            ...
            if key == old_key and year == old_year:
                if month != old_month:
                    for i in range(5):
                        if feature[i].size > 0:
                            out[int(old_month)-1][i*2] = feature[i].mean()
                            out[int(old_month)-1][i*2+1] = feature[i].var()
                        feature[i] = np.array([])
                    old_month = month
            else:
                tmp = out.flatten()
                outstring = old_key+'\t'+ old_year+'\t'+ str(tmp[0])+','+ str(tmp[1])
                for i in range(2,120,2):
                    outstring += ','+str(tmp[i])+','+str(tmp[i+1])
                print outstring
                old_key = key
                old_year = year
                out = np.zeros(shape=(12,10))
